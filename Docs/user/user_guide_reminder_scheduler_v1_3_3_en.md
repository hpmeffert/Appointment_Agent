...
- `Sync`
  Shows the sync window, hash detection, and idempotency guard.
- `Help`
...
- `sync_window_days`
  How many days ahead the adapter looks for changes.
- `polling_interval_minutes`
  How often the adapter checks for updates.
- `hash_detection_enabled`
  Turns hash checks on or off.
- `idempotency_guard`
  Prevents duplicate reminder jobs when the same appointment is seen again.
...
## Sync and change detection in simple words

The adapter checks for appointments again and again.

That matters because:
- a new appointment should create reminders once
- a changed appointment should update the reminder plan
- a cancelled appointment should stop the reminder plan

The `Sync` page shows:
- the sync window
- if hash detection is enabled
- if duplicate jobs are blocked
...
When an appointment changes, the scheduler should react in a predictable way.

This helps the operator:
- see why the plan changed
- keep the reminder list free of duplicates
- trust that a cancelled appointment will stop the reminder plan
