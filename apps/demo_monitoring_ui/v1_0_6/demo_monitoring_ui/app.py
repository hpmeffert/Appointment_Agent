...
router = APIRouter(tags=["demo-cockpit-v1.0.6"])
...
@router.get("/ui/demo-monitoring/v1.0.6")
def ui_view() -> HTMLResponse:
    return HTMLResponse(_render_html("v1.0.6"))


@router.get("/api/demo-monitoring/v1.0.6/help")
def help_view(lang: str = Query(default="en")) -> dict:
    payload = build_cockpit_payload(version="v1.0.6", lang="de" if lang == "de" else "en", include_verticals=True)
    return {
...
@router.get("/api/demo-monitoring/v1.0.6/payload")
def payload_view(lang: str = Query(default="en")) -> dict:
    return build_cockpit_payload(version="v1.0.6", lang="de" if lang == "de" else "en", include_verticals=True)
