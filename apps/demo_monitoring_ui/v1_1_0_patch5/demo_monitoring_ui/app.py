...
from .payloads import build_patch5_payload

router = APIRouter(tags=["demo-monitoring-ui-v1.1.0-patch5"])

HTML_PATH = Path(__file__).resolve().parent / "static" / "cockpit.html"


def _render_html() -> str:
    return HTML_PATH.read_text(encoding="utf-8").replace("__VERSION__", "v1.1.0-patch5")


@router.get("/ui/demo-monitoring/v1.1.0-patch5")
def ui_view() -> HTMLResponse:
    return HTMLResponse(_render_html())


@router.get("/api/demo-monitoring/v1.1.0-patch5/help")
def help_view(lang: str = Query(default="en")) -> dict:
    payload = build_patch5_payload(lang="de" if lang == "de" else "en")
    return {
...
@router.get("/api/demo-monitoring/v1.1.0-patch5/payload")
def payload_view(lang: str = Query(default="en")) -> dict:
    return build_patch5_payload(lang="de" if lang == "de" else "en")
