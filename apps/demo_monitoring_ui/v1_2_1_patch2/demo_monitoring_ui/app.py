...
from .payloads import build_v121_patch2_payload

router = APIRouter(tags=["demo-monitoring-ui-v1.2.1-patch2"])
...
def _render_html() -> str:
    return (
        ...
        .replace("__VERSION__", "v1.2.1-patch2")
        .replace('const GOOGLE_API_VERSION = "v1.1.0-patch8b";', 'const GOOGLE_API_VERSION = "v1.2.0";')
        .replace('const LEKAB_API_VERSION = "v1.2.1";', 'const LEKAB_API_VERSION = "v1.2.1-patch2";')
    )


@router.get("/ui/demo-monitoring/v1.2.1-patch2")
def ui_view() -> HTMLResponse:
    return HTMLResponse(_render_html())


@router.get("/api/demo-monitoring/v1.2.1-patch2/help")
def help_view(lang: str = Query(default="en")) -> dict:
    payload = build_v121_patch2_payload(lang="de" if lang == "de" else "en")
    return {
...
@router.get("/api/demo-monitoring/v1.2.1-patch2/payload")
def payload_view(lang: str = Query(default="en")) -> dict:
    return build_v121_patch2_payload(lang="de" if lang == "de" else "en")
