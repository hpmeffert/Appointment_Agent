...
def build_v130_payload(lang: str = "en") -> dict:
    ...
    payload = {
        ...
        "ui_meta": {
            "tagline": (
                "Reminder Scheduler Setup UI with clear policies, preview, and safe planning."
                if language == "en"
                else "Reminder Scheduler Setup UI mit klaren Regeln, Vorschau und sicherer Planung."
            ),
            "accent": "amber",
            "role": "admin-console",
        },
        ...
        "scheduler_overview": {
            ...
            "subtitle": (
                "The system reads appointments, calculates reminder times, creates jobs, and dispatches them safely."
                if language == "en"
                else "Das System liest Termine, berechnet Erinnerungszeiten, erzeugt Jobs und versendet sie sicher."
            ),
            "cards": [
                {
        ...
        "demo_stories": [
            ...
            },
            {
                ...
                "business_value": (
                    "A team can define 72h, 48h, and 24h reminders manually."
                    if language == "en"
                    else "Ein Team kann 72h, 48h und 24h Erinnerungen manuell festlegen."
                ),
                "wow_effect": (
            ...
            },
            {
                ...
                "business_value": (
                    "The system can spread reminders automatically when the operator only defines the count and constraints."
                    if language == "en"
                    else "Das System kann Erinnerungen automatisch verteilen, wenn nur Anzahl und Regeln definiert sind."
                ),
                "wow_effect": (
        ...
        "ui_api_links": [
            "/api/reminder-ui/v1.3.0/payload",
            "/api/reminder-ui/v1.3.0/help",
            "/api/reminder-ui/v1.3.0/config",
            "/api/reminder-ui/v1.3.0/config/preview",
            "/api/reminder-ui/v1.3.0/jobs",
        ],
        "api_links": [
            "/api/reminders/v1.3.0/config",
            "/api/reminders/v1.3.0/config/preview",
            "/api/reminders/v1.3.0/jobs",
            "/api/reminders/v1.3.0/rebuild",
            "/api/reminders/v1.3.0/health",
        ],
        "docs_highlights": [
            (
