...
def build_v132_payload(lang: str = "en") -> dict:
    language = "de" if lang == "de" else "en"
    payload = {
        "version": "v1.3.2",
        "ui_meta": {
            "tagline": (
                "Reminder Scheduler Setup UI with clear policies, lifecycle visibility, preview, safe planning, and time awareness."
                if language == "en"
                else "Reminder Scheduler Setup UI mit klaren Regeln, Lifecycle-Sichtbarkeit, Vorschau, sicherer Planung und Zeitbewusstsein."
            ),
            "accent": "amber",
            "role": "admin-console",
        },
        "pages": [
            ...
            {"id": "time", "label": "Time" if language == "en" else "Zeit"},
        ...
        "scheduler_overview": {
            ...
            "subtitle": (
                "The system reads appointments, calculates reminder times, keeps timezone and DST in view, creates jobs, shows lifecycle states, and dispatches them safely."
                if language == "en"
                else "Das System liest Termine, berechnet Erinnerungszeiten, behält Zeitzone und Sommerzeit im Blick, erzeugt Jobs, zeigt Lifecycle-Zustaende und versendet sie sicher."
            ),
            "cards": [
                {
                ...
                {
                    "title": "Time Awareness" if language == "en" else "Zeit-Bewusstsein",
                    "body": (
                        "The cockpit shows timezone, DST, and near-term reminder timing so the operator can spot risky reminders quickly."
                        if language == "en"
                        else "Das Cockpit zeigt Zeitzone, Sommerzeit und nah bevorstehende Erinnerungen, damit der Operator riskante Zeitpunkte schnell sieht."
                    ),
                },
                {
        ...
        "validation_rules": [
            ...
            "timezone and DST checks stay visible for near-term reminders",
        ],
        "demo_stories": [
            {
                "id": "single-reminder",
        ...
        "time_awareness": {
            ...
            "notes": [
                (
                    "Reminders close to the current time are highlighted so the operator can check them first."
                    if language == "en"
                    else "Erinnerungen nahe an der aktuellen Zeit werden markiert, damit der Operator sie zuerst pruefen kann."
                ),
                (
                    "The scheduler keeps local time and DST in mind when it shows the final send time."
                    if language == "en"
                    else "Der Scheduler berücksichtigt lokale Zeit und Sommerzeit, wenn er die Endzeit zeigt."
                ),
            ],
        },
        "lifecycle_states": [
        ...
        "ui_api_links": [
            "/api/reminder-ui/v1.3.2/payload",
            "/api/reminder-ui/v1.3.2/help",
            "/api/reminder-ui/v1.3.2/config",
            "/api/reminder-ui/v1.3.2/config/preview",
            "/api/reminder-ui/v1.3.2/jobs",
        ],
        "api_links": [
            "/api/reminders/v1.3.2/config",
            "/api/reminders/v1.3.2/config/preview",
            "/api/reminders/v1.3.2/jobs",
            "/api/reminders/v1.3.2/rebuild",
            "/api/reminders/v1.3.2/health",
        ],
        "docs_highlights": [
            (
