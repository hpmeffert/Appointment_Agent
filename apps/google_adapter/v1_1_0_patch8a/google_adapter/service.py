...
from appointment_agent_shared.repositories import BookingRepository, SlotHoldRepository
...
class GoogleSlotHoldCreateRequest(BaseModel):
    mode: Literal["simulation", "test"] = "simulation"
    journey_id: str
    ...
    end_time: datetime
    slot_label: str
    hold_duration_minutes: Optional[int] = Field(default=None, ge=1, le=30)
    reason: Optional[str] = "slot_selected"


class GoogleSlotHoldReleaseRequest(BaseModel):
    journey_id: str
    hold_id: str
    reason: Optional[str] = "user_cancelled"


class GoogleSlotHoldResult(BaseModel):
    success: bool
...
class GoogleAdapterServiceV110Patch8A(GoogleAdapterServiceV110Patch8):
    def __init__(self, session: Session) -> None:
        super().__init__(session)
    ...
    def _effective_hold_duration(self, override_minutes: Optional[int]) -> int:
        return override_minutes or settings.slot_hold_minutes

    def _normalize_utc(self, value: datetime) -> datetime:
        aware = value if value.tzinfo is not None else value.replace(tzinfo=timezone.utc)
        return aware.astimezone(timezone.utc).replace(tzinfo=None)

    def _active_hold_for_slot(self, *, slot_id: str, start_time: datetime, end_time: datetime, journey_id: Optional[str] = None):
        holds = self.slot_holds.find_active_for_slot(
            slot_id=slot_id,
            start_time_utc=self._normalize_utc(start_time),
            end_time_utc=self._normalize_utc(end_time),
        )
        if journey_id:
            holds = [hold for hold in holds if hold.journey_id != journey_id]
        return holds

    def create_slot_hold_patch8a(self, request: GoogleSlotHoldCreateRequest) -> GoogleSlotHoldResult:
        # Holds live in the platform state so parallel users cannot race through
        # the same slot just because the provider still reports it as free.
        ...
        self.slot_holds.release_for_journey(request.journey_id)
        active_conflicts = self._active_hold_for_slot(
        ...
        if active_conflicts:
            conflicting_hold = active_conflicts[0]
            return GoogleSlotHoldResult(
                success=False,
                mode=status.mode,
                ...
                hold_status="ACTIVE",
                hold_duration_minutes=self._effective_hold_duration(request.hold_duration_minutes),
                expires_at_utc=conflicting_hold.expires_at_utc.replace(tzinfo=timezone.utc).isoformat(),
                message="This slot is temporarily reserved by another journey.",
                conflict_detected=True,
                monitoring_labels=["slot.temporarily_reserved", "booking.blocked.by_parallel_request"],
                technical_reason="slot.temporarily_reserved",
            )
        hold_minutes = self._effective_hold_duration(request.hold_duration_minutes)
        created_at_utc = datetime.utcnow()
        expires_at_utc = created_at_utc + timedelta(minutes=hold_minutes)
        hold_id = "hold-{}".format(uuid4().hex[:10])
        self.slot_holds.create(
            hold_id=hold_id,
            journey_id=request.journey_id,
            ...
            provider="google",
            slot_label=request.slot_label,
            start_time_utc=self._normalize_utc(request.start_time),
            end_time_utc=self._normalize_utc(request.end_time),
        ...
        return GoogleSlotHoldResult(
            ...
            expires_at_utc=expires_at_utc.replace(tzinfo=timezone.utc).isoformat(),
            message="Slot hold created successfully.",
            monitoring_labels=["slot.hold.created", "slot.hold.active"],
        )

    def release_slot_hold_patch8a(self, request: GoogleSlotHoldReleaseRequest) -> GoogleSlotHoldResult:
        self._expire_stale_holds()
        existing_hold = self.slot_holds.get_active_by_hold_id(request.hold_id)
        if existing_hold is not None and existing_hold.journey_id != request.journey_id:
            raise GoogleAdapterException(
                ProviderError(
                    provider="google",
                    provider_operation="release_slot_hold_patch8a",
                    error_category=ErrorCategory.VALIDATION,
                    message="Slot hold belongs to a different journey.",
                )
            )
        released = self.slot_holds.release(request.hold_id, status="RELEASED")
        if released is None:
            raise GoogleAdapterException(
                ProviderError(
                    provider="google",
        ...
        return GoogleSlotHoldResult(
            ...
            mode="simulation",
            google_source="simulation",
            slot_id=released.slot_id,
            slot_label=released.slot_label or released.slot_id,
            hold_status="RELEASED",
            hold_duration_minutes=max(int((released.expires_at_utc - released.created_at).total_seconds() // 60), 0),
            expires_at_utc=released.expires_at_utc.replace(tzinfo=timezone.utc).isoformat(),
            message="Slot hold released.",
            monitoring_labels=["slot.hold.released"],
        )
    ...
    def get_available_slots_patch8(self, request: GoogleAvailabilitySlotsRequest) -> GoogleAvailabilitySlotsResult:
        ...
        for index, (start_time, end_time) in enumerate(
            ...
            if self._active_hold_for_slot(slot_id="patch8-slot-{}".format(index), start_time=start_time, end_time=end_time):
                continue
        ...
        return GoogleAvailabilitySlotsResult(
            ...
            slots=slots,
            monitoring_labels=["slot.checked", "slot.list.generated", "slot.hold.filtered"],
        )

    def create_booking_patch8a(self, request: GoogleBookingCreateRequest, *, journey_id: str, hold_id: str) -> GoogleBookingActionResult:
        self._expire_stale_holds()
        hold = self.slot_holds.get_active_by_hold_id(hold_id)
        if (
            hold is None
            or hold.journey_id != journey_id
            or hold.slot_id != request.slot_id
            or hold.start_time_utc != self._normalize_utc(request.start_time)
            or hold.end_time_utc != self._normalize_utc(request.end_time)
        ):
            return GoogleBookingActionResult(
                success=False,
                action="create",
                mode=request.mode,
                google_source="live" if request.mode == "test" else "simulation",
                message="Slot hold expired or is no longer active.",
                status="hold_expired",
                conflict_detected=True,
                monitoring_labels=["slot.hold.expired", "booking.blocked.by_parallel_request"],
                technical_reason="slot.hold.expired",
            )
        result = self.create_booking_patch8(request)
        if result.success:
            self.slot_holds.release(hold_id, status="CONSUMED")
            result.monitoring_labels.append("slot.hold.consumed")
        return result

    def reschedule_booking_patch8a(self, request: GoogleBookingRescheduleRequest, *, journey_id: str, hold_id: str) -> GoogleBookingActionResult:
        self._expire_stale_holds()
        hold = self.slot_holds.get_active_by_hold_id(hold_id)
        if (
            hold is None
        ...
        result = self.reschedule_booking_patch8(request)
        if result.success:
            self.slot_holds.release(hold_id, status="CONSUMED")
            result.monitoring_labels.append("slot.hold.consumed")
        return result
    ...
    def get_available_slots_patch8(self, request: GoogleAvailabilitySlotsRequest) -> GoogleAvailabilitySlotsResult:
        ...
        for index, (start_time, end_time) in enumerate(
            ...
            slot_id = "patch8-slot-{}".format(index)
            if self._active_hold_for_slot(slot_id=slot_id, start_time=start_time, end_time=end_time):
                continue
            conflicts = self._load_conflicts_for_window(
                status=status,
                start_time=start_time,
