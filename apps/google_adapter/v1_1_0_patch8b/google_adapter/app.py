...
from google_adapter.v1_1_0_patch8b.google_adapter.service import GoogleAdapterServiceV110Patch8B

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/google/v1.1.0-patch8b", tags=["google-adapter-v1.1.0-patch8b"])
...
def get_service(session: Session = Depends(get_session)) -> GoogleAdapterServiceV110Patch8B:
    return GoogleAdapterServiceV110Patch8B(session)
...
def mode_status(
    ...
    service: GoogleAdapterServiceV110Patch8B = Depends(get_service),
) -> dict:
...
def contact_linking(service: GoogleAdapterServiceV110Patch8B = Depends(get_service)) -> dict:
