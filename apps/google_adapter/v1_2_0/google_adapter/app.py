...
router = APIRouter(prefix="/api/google/v1.2.0", tags=["google-adapter-v1.2.0"])
...
def get_service(session: Session = Depends(get_session)) -> GoogleAdapterServiceV120:
    return GoogleAdapterServiceV120(session)
...
def mode_status(
    ...
    service: GoogleAdapterServiceV120 = Depends(get_service),
) -> dict:
...
def contact_linking(service: GoogleAdapterServiceV120 = Depends(get_service)) -> dict:
