...
router = APIRouter(prefix="/api/reminders/v1.3.2", tags=["reminder-scheduler-v1.3.2"])
app = FastAPI(title="Appointment Reminder Scheduler v1.3.2")
...
def preview_current_config(
    ...
    # A read-only preview endpoint is useful in demos and operations because it
    # shows the current scheduling result without mutating persistence.
    return service.get_config(policy_key=policy_key).model_dump(mode="json")
...
def root_view() -> dict[str, object]:
    return {
        "module": "reminder_scheduler",
        "version": "v1.3.2",
        "help_path": "/api/reminders/v1.3.2/help",
        "health_path": "/api/reminders/v1.3.2/health",
    }
