...
from . import calculator, time_utils, validation
...
class ReminderSchedulerService:
    ...
    def save_appointment_cache(
        ...
        *,
        ...
        for appointment in appointments:
            ...
            # That keeps rebuilds deterministic across timezones and makes DST
            # edge cases reproducible in tests.
    ...
    def _deliver_job(self, record: ReminderJobRecord) -> str:
        # Delivery remains simulated in this release line. The important part
        # is the lifecycle around lock acquisition, retries, and the UTC-based
        # schedule that feeds this hook.
        ...
        *,
    ...
    ) -> tuple[list[ReminderJobRecord], int, int, int, int]:
        ...
        for item in calculation.items:
            ...
            if existing is not None:
                if existing.status == ReminderStatus.DISPATCHING.value and existing.locked_until_utc and existing.locked_until_utc > time_utils.utcnow():
                    skipped += 1
                    jobs.append(existing)
                    self._append_audit(
                        tenant_id=appointment.tenant_id,
        ...
        return jobs, created, updated, skipped, skipped_past
    ...
    def get_config(self, policy_key: str = "global") -> ReminderConfigView:
        ...
        return ReminderConfigView(
            ...
            preview=[self._calculated_item_to_preview(item) for item in calculation.items[:10]],
    ...
    def preview(self, request: ReminderPreviewRequest) -> ReminderConfigView:
        ...
        return ReminderConfigView(
            ...
            validation=validation_result,
            warnings=validation_result,
            preview=[self._calculated_item_to_preview(item) for item in calculation.items],
            skipped_preview_items=calculation.skipped_count,
            job_count=0,
    ...
    def rebuild(self, request: ReminderRebuildRequest) -> dict[str, Any]:
        ...
        planned, created, updated, skipped, skipped_past = self._plan_jobs(
            policy,
    ...
    def plan_jobs(self, policy_key: str = "global") -> ReminderPlanningResult:
        ...
        return ReminderPlanningResult(
            ...
            appointment_count=len(appointments),
            planned_preview=[self._calculated_item_to_preview(item) for item in calculation.items],
        )
