...
class ReminderSchedulerService:
    ...
    def _list_appointments(
        ...
        for row in rows:
            linkage = self.address_linkage.resolve(
                ...
                appointment_id=row.external_appointment_id,
                booking_reference=(row.raw_payload_json or {}).get("booking_reference"),
            ...
            appointments.append(
                ReminderAppointmentInput(
                    ...
                    email=row.email,
                    phone=row.phone,
                    address_id=row.address_id or linkage.address_id,
                    correlation_ref=row.correlation_ref or linkage.correlation_ref,
                    status=row.status,
    ...
    def _normalized_appointments(
        ...
        normalized = validation.normalize_appointments(appointments)
        enriched: list[ReminderAppointmentInput] = []
        for appointment in normalized:
            if appointment.address_id and appointment.correlation_ref:
                enriched.append(appointment)
                continue
            linkage = self.address_linkage.resolve(
                tenant_id=appointment.tenant_id,
                appointment_id=appointment.appointment_external_id,
                booking_reference=str(appointment.metadata.get("booking_reference") or "") or None,
                correlation_ref=appointment.correlation_ref,
            ...
            enriched.append(
                appointment.model_copy(
                    update={
                        "address_id": appointment.address_id or linkage.address_id,
                        "correlation_ref": appointment.correlation_ref or linkage.correlation_ref,
                    }
                )
            )
        return enriched
        ...
        *,
        ...
        for item in calculation.items:
            ...
            if existing is not None:
                ...
                existing.address_id = appointment.address_id
                existing.correlation_ref = appointment.correlation_ref
