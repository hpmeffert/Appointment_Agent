...
from demo_monitoring_ui.v1_3_5.demo_monitoring_ui.app import app


def test_reminder_scheduler_v135_ui_and_api_routes_load() -> None:
    client = TestClient(app)

    ui_response = client.get("/ui/reminder-scheduler/v1.3.5")
    payload_response = client.get("/api/reminder-ui/v1.3.5/payload")
    help_response = client.get("/api/reminder-ui/v1.3.5/help")
    config_response = client.get("/api/reminder-ui/v1.3.5/config")
    preview_response = client.get("/api/reminder-ui/v1.3.5/config/preview")
    results_response = client.get("/api/reminder-ui/v1.3.5/results")
    health_response = client.get("/api/reminder-ui/v1.3.5/health")

    assert ui_response.status_code == 200
    assert "Appointment Agent Reminder Scheduler v1.3.5" in ui_response.text
    ...
    assert preview_payload["version"] == "v1.3.5"
    ...
    assert results_payload["version"] == "v1.3.5"
    ...
    assert health_payload["version"] == "v1.3.5"
...
def test_reminder_scheduler_v135_supports_german_language() -> None:
    client = TestClient(app)

    ui_response = client.get("/ui/reminder-scheduler/v1.3.5")
    help_response = client.get("/api/reminder-ui/v1.3.5/help?lang=de")
    payload_response = client.get("/api/reminder-ui/v1.3.5/payload?lang=de")

    assert ui_response.status_code == 200
    assert "Delivery-style reminder cockpit" in ui_response.text
    assert help_response.status_code == 200
    assert help_response.json()["version"] == "v1.3.5"
    ...
    assert payload_response.status_code == 200
    assert payload_response.json()["version"] == "v1.3.5"
