...
from reminder_scheduler.v1_3_4.reminder_scheduler import adapter, calculator
from reminder_scheduler.v1_3_4.reminder_scheduler.app import app
from reminder_scheduler.v1_3_4.reminder_scheduler.service import (
...
from reminder_scheduler.v1_3_4.reminder_scheduler.worker import ReminderSchedulerWorker


DB_PATH = Path(gettempdir()) / f"appointment_agent_reminder_v134_{uuid4().hex}.db"
...
def test_v134_help_root_and_features_show_sync_contract_and_timezone_guards() -> None:
    client = _client()

    help_response = client.get("/api/reminders/v1.3.4/help")
    root_response = client.get("/api/reminders/v1.3.4/")
    ...
    assert root_response.json()["version"] == "v1.3.4"


def test_v134_hash_is_stable_for_equal_inputs_and_changes_when_schedule_changes() -> None:
...
def test_v134_repeated_rebuild_runs_are_idempotent() -> None:
    client = _client()
    ...
    jobs_response = client.get("/api/reminders/v1.3.4/jobs")
    jobs_payload = jobs_response.json()
    assert jobs_payload["count"] == 2
    assert {job["status"] for job in jobs_payload["jobs"]} == {ReminderStatus.PLANNED.value}


def test_v134_rebuild_updates_existing_jobs_when_appointment_time_changes() -> None:
    client = _client()
    ...
    jobs_payload = client.get("/api/reminders/v1.3.4/jobs").json()
    reschedule_jobs = [
...
def test_v134_cancelled_appointment_reconciles_existing_reminders() -> None:
    client = _client()
...
def test_v134_rejects_ambiguous_and_nonexistent_naive_times() -> None:
    client = _client()
    policy = _manual_policy("naive-block", reminder_count=1)

    ambiguous_response = client.post(
        "/api/reminders/v1.3.4/config/preview",
        json={
            "policy": policy.model_dump(mode="json"),
            "appointments": [
                {
...
def test_v134_worker_and_preview_use_the_same_calculation_path(monkeypatch: pytest.MonkeyPatch) -> None:
    session = TestingSessionLocal()
    try:
...
def test_v134_lifecycle_guards_remain_strict() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
...
def test_v134_dispatch_marks_invalid_email_targets_as_skipped() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
...
def test_v134_dispatch_maps_retryable_delivery_failures_back_to_planned() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
...
def test_v134_dispatch_maps_terminal_voice_failures_to_failed() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
...
def test_v134_dispatch_supports_email_voice_and_rcs_sms_success_paths() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
