...
DB_PATH = f"{gettempdir()}/appointment_agent_reminder_v134_delivery_{uuid4().hex}.db"
...
def test_v134_dispatch_success_marks_job_sent() -> None:
...
def test_v134_retryable_dispatch_failure_requeues_job() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
...
def test_v134_terminal_dispatch_failure_for_missing_target() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
...
def test_v134_policy_channel_disable_blocks_delivery() -> None:
    session = TestingSessionLocal()
    try:
        service = ReminderSchedulerService(session)
